...
function renderLibrary(){
  ...
  const matches = phrases.filter(p => (selectedCategory === 'All' || p.category === selectedCategory) && `${p.cebuano} ${p.english} ${p.situation}`.toLowerCase().includes(query));
...
renderFilters(); renderLibrary(); renderPractice(); updateProgress();
